---
caption: #ana sayfa ve /islerim/ grid'inde gorunen bilgiler
  title: PVC Teras Kapama
  subtitle: Turhal / Kış Bahçesi
  thumbnail: https://i.pinimg.com/1200x/53/24/0b/53240bd86a7cb18f5695497180d30284.jpg

#proje sayfasinda gorunen bilgiler
title: PVC Teras Kapatma ve Kış Bahçesi
subtitle: Turhal
location: "Tokat / Turhal"
ilce: "Turhal"
date: 2025-10-01
date_text: "Ekim 2025"
client: "Kemal Bey"
category: "PVC Sistemleri"
alt: Turhal müstakil ev sandviç panel çatılı ısıcamlı PVC teras kapatma ve kış bahçesi uygulaması
description: "Turhal'da müstakil evin terasını sandviç panel çatı ve 80'lik seri PVC doğrama ile kapatarak ısıcamlı kış bahçesine dönüştürdüğümüz proje."
---
## Proje Hakkında

Turhal'da müstakil bir evin açık terasını kapalı kış bahçesine dönüştürdük. Teras yılın büyük bölümünde kullanılmıyordu; yazın aşırı sıcak, kışın ise tamamen atıl duruyordu.

Teras kapatmada asıl belirleyici olan çatıdır. Sadece yanları camlayıp üstü açık bırakılan ya da tek kat saç ile kapatılan uygulamalar, yazın sera etkisiyle dayanılmaz sıcaklığa ulaşır. Bu yüzden çatıyı yalıtımlı panelle çözdük.

### Teknik Detaylar

* **Sandviç Panel Çatı:** İki metal yüzey arasına 4 cm poliüretan dolgulu paneller kullanıldı. Yazın güneş ısısını, kışın içerideki sıcaklığı tutar. Yağmur sesini de belirgin ölçüde azaltır.
* **80'lik Seri PVC Profil:** Çatı yükünü taşıyacak profil seçimi kritikti. Standart 60'lık seri yerine et kalınlığı ve odacık sayısı yüksek 80'lik seri tercih edildi.
* **Çelik Takviye:** Dikey taşıyıcı profillerin içine galvanizli çelik takviye geçirildi. PVC tek başına taşıyıcı değildir; yükü içindeki çelik taşır.
* **Isıcam Sinerji Seri:** Yan cephelerde düşük ısı geçirgenlikli cam kullanıldı. Kış bahçesi mekanik ısıtma olmadan da dış ortamdan belirgin şekilde ılık kalıyor.
* **Eğimli Çatı ve Dere:** Çatıya su birikmesini önleyecek eğim verildi, tahliye evin mevcut yağmur iniş borusuna bağlandı.

### Sonuç

Atıl duran teras, evin en çok vakit geçirilen odasına dönüştü. Ev sahibi kış aylarında burayı kahvaltı alanı olarak kullandığını, yaz aylarında ise yan kanatları açarak yarı açık teras düzenine geçebildiğini belirtti.
